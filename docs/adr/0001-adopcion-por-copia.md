...
La capa se adopta copiando su inventario canónico al proyecto: `npx` ejecuta el
inicializador una vez y no queda ningún vínculo posterior. Decidimos que el
...
- **Copia con reejecución idempotente** (elegida). El mismo comando revalida,
  informa divergencias y ofrece reemplazo explícito.

## Consecuencias

Actualizar es volver a ejecutar el comando: los archivos idénticos se validan,
los ausentes se copian y los divergentes exigen decisión. Reemplazar pierde las
modificaciones locales de los archivos canónicos listados. No existe forma de
saber si una capa instalada está al día sin ejecutar el inicializador, y de ahí
que exista `.agents/VERSION` como única marca de la versión adoptada.
